...
absl::Status SetSocketSendBuf(FileDescriptor fd, int buffer_size_bytes) {
  return 0 == g_event_poller->GetSystemApi()->SetSockOpt(
                  fd, SOL_SOCKET, SO_SNDBUF, &buffer_size_bytes,
...
void CreateTestSocket(int port, FileDescriptor* socket_fd,
                      struct sockaddr_in6* sin) {
  int one = 1;
  int buffer_size_bytes = BUF_SIZE;
  int flags;

  SystemApi* api = g_event_poller->GetSystemApi();
  FileDescriptor fd = api->Socket(AF_INET6, SOCK_STREAM, 0);
  api->SetSockOpt(fd, SOL_SOCKET, SO_REUSEADDR, &one, sizeof(one));
  ...
  flags = api->Fcntl(fd, F_GETFL, 0);
  EXPECT_EQ(api->Fcntl(fd, F_SETFL, flags | O_NONBLOCK), 0);
  *socket_fd = fd;
...
void SessionReadCb(session* se, absl::Status status) {
  ...
  do {
    read_once =
        g_event_poller->GetSystemApi()->Read(fd, se->read_buf, BUF_SIZE);
...
void ClientSessionWrite(client* cl, absl::Status status) {
  ...
  do {
    write_once = cl->em_fd->Poller()->GetSystemApi()->Write(
        fd, cl->write_buf, CLIENT_WRITE_BUF_SIZE);
...
void ClientStart(client* cl, int port) {
  ...
  struct sockaddr_in6 sin;
  CreateTestSocket(port, &fd, &sin);
  if (g_event_poller->GetSystemApi()->Connect(
          fd, reinterpret_cast<struct sockaddr*>(&sin), sizeof(sin)) == -1) {
...
TEST_F(EventPollerTest, TestEventPollerHandleChange) {
  ...
  flags = system_api->Fcntl(std::get<1>(code_sv0_sv1), F_GETFL, 0);
  EXPECT_EQ(
      system_api->Fcntl(std::get<1>(code_sv0_sv1), F_SETFL, flags | O_NONBLOCK),
      0);
  flags = system_api->Fcntl(std::get<1>(code_sv0_sv1), F_GETFL, 0);
  EXPECT_EQ(
      system_api->Fcntl(std::get<1>(code_sv0_sv1), F_SETFL, flags | O_NONBLOCK),
  ...
  em_fd = g_event_poller->CreateHandle(std::get<1>(code_sv0_sv1),
                                       "TestEventPollerHandleChange", false);
  EXPECT_NE(em_fd, nullptr);
  // Register the first callback, then make its FD readable
  em_fd->NotifyOnRead(first_closure);
  data = 0;
  result = system_api->Write(std::get<1>(code_sv0_sv1), &data, 1);
  EXPECT_EQ(result, 1);
  ...
  result = system_api->Read(std::get<1>(code_sv0_sv1), &data, 1);
  EXPECT_EQ(result, 1);
  ...
  DestroyChangeData(&a);
  DestroyChangeData(&b);
  system_api->Close(std::get<1>(code_sv0_sv1));
}
