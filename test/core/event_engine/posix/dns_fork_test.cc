 ...
 private:
  void ServerLoop(int sockfd) {
    ...
    while (!done_.HasBeenNotified()) {
      ...
      auto query =
          ParseDnsQuery(absl::Span<const char>(buffer).first(received_bytes));
      if (!query.ok()) {
  ...
  std::string ParseQName(absl::Span<const char> buffer, size_t& pos) {
  ...
  // Note that this needs cleanup, may read past the buffer.
  absl::StatusOr<DnsQuery> ParseDnsQuery(absl::Span<const char> buffer) {
    DnsQuery query;
    size_t offset = 0;
