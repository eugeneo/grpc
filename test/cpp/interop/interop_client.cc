...
bool InteropClient::DoOrcaPerRpc() {
  auto load_report_tracker = std::make_shared<LoadReportTracker>();
  grpc_core::CoreConfiguration::RegisterBuilder(
      std::bind(RegisterBackendMetricsLbPolicy, std::placeholders::_1,
                load_report_tracker));
  ...
  auto report = load_report_tracker->GetNextLoadReport();
  ...
  GPR_ASSERT(!load_report_tracker->GetNextLoadReport().has_value());
