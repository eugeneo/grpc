...
absl::Status PrepareSocket(const SystemApi& system_api,
  ...
  if (PosixSocketWrapper::IsSocketReusePortSupported(system_api) &&
      options.allow_reuse_port && socket.addr.address()->sa_family != AF_UNIX &&
      !ResolvedAddressIsVSock(socket.addr)) {
    GRPC_RETURN_IF_ERROR(socket.sock.SetSocketReusePort(system_api, 1));
  }

#ifdef GRPC_LINUX_ERRQUEUE
  if (!socket.sock.SetSocketZeroCopy(system_api).ok()) {
  ...
  GRPC_RETURN_IF_ERROR(socket.sock.SetSocketNonBlocking(system_api, 1));
  GRPC_RETURN_IF_ERROR(socket.sock.SetSocketCloexec(system_api, 1));

  if (socket.addr.address()->sa_family != AF_UNIX &&
      !ResolvedAddressIsVSock(socket.addr)) {
    GRPC_RETURN_IF_ERROR(socket.sock.SetSocketLowLatency(system_api, 1));
    GRPC_RETURN_IF_ERROR(socket.sock.SetSocketReuseAddr(system_api, 1));
