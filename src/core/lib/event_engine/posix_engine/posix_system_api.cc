...
bool PosixSystemApi::IsSocketReusePortSupported() const {
...
absl::Status PosixSystemApi::SetSocketReusePort(FileDescriptor fd,
                                                int reuse) const {
#ifndef SO_REUSEPORT
  return absl::Status(absl::StatusCode::kInternal,
...
absl::Status PosixSystemApi::SetSocketReuseAddr(FileDescriptor fd,
                                                int reuse) const {
  int val = (reuse != 0);
  int newval;
  socklen_t intlen = sizeof(newval);
